...
if st.button("Generate Campaign"):
    with st.spinner("AI Agents are building your campaign (11 agents running)..."):
        ...
        social_manager = Agent(
            ...
            goal=f"Translate the written campaign assets into high-performance social posts for the selected platforms: {', '.join(platforms)}. For Instagram, ensure an image suggestion prompt is included.",
        ...
        social_task = Task(
            description=f"Review the generated blog article and write social media promotions for platforms: {', '.join(platforms)}. Create 1 LinkedIn post using the 'hook -> story -> value -> CTA' structure, a Twitter/X thread of 4-6 tweets, and for Instagram, create a caption and suggest an image to accompany it. IMPORTANT: For Instagram, write the image suggestion in this exact format: 'IMAGE_PROMPT: [Detailed description of a highly relevant image to generate]'.",
